...
The final design review must settle the delivery endpoint (including whether to publish releases/images), deployment/support scope and operational trade-offs. Record those answers in the [design/handoff decision](https://github.com/djosh34/cnpg_backup/issues/14) before setting it READY. No production database/bucket operations belong to implementation or CI. Respect existing platform protections; verify before handoff that required credentials/rules allow the agreed autonomous delivery instead of leaving a predictable human step for later.
...
4. Verify Paseo Astra/high dispatch/result retrieval/archival, git/Actions permissions, merge-rule compatibility and selected artifact publication credentials if publication is part of the agreed endpoint. Complete this preflight before launch.
...
Repeat through the entire graph. Exercise recovery continuously, not just at the end. Qualify exact immutable candidate images with the real-system campaign, DST/fuzz corpus, restore SQL assertions and upgrade fixtures. Produce install/recovery runbooks and version/resource evidence. Complete the agreed delivery endpoint automatically; if release publication is included, use the agreed tag/version and qualified digests without an extra approval ceremony.
