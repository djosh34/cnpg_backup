...
Default native range: **0/2000028–0/2000120**, target `postbackup_target` in later archived WAL. Same-segment captured range: **0/4057538–0/4401660**, timeline 1; real restore point **r6_7595** starts at **0/4401660** (its record extends beyond End-LSN) in **000000010000000000000004**. Full archive replay stopped at that point; the padded fixture could not. The original archive and bundle were identical, 16,777,216 bytes, SHA256 `dfc0178fd70e29855e5280e8c99141c73d661fc7e25553bcfb5b78c08f1af138`. The researcher **synthetically zeroed bytes from End-LSN to segment end**, preserving the manifest range; padded SHA256 `599f3dac56a6ceab16ebbf44e52382ade2e089e7f6fd5e13001e4a1e74ef312f`. This is controlled padding, **not naturally captured padding**. Original fixture, archive and WAL dump were retained in `/tmp/recovery-review-pg-_db70vra`; the parent also preserved the selected inputs as the checksum-pinned [portable regression fixture](fixtures/README.md), so evidence does not depend on that temporary directory. The final script replays it without a race workload. Default script requires no retained fixture.
...
- The parent's first hosted portability run [34074136064](https://github.com/djosh34/cnpg_backup/actions/runs/34074136064) passed all default-fixture controls, then rejected the retained fixture's SQL login: the hosted OS user `runner` is not the captured database role `starlord`. The script now explicitly selects that test-only role for retained inputs and initializes new fixtures with fixed `cnpg_research`; no authentication oracle is relaxed. Hosted results are supplemental mechanism evidence, never product qualification.
