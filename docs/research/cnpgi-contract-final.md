...
| Authenticated scoped `NoSuchKey` for optional history/future tail, or an archive duplicate with verified bundled-local fallback and no required remote bytes in that segment | **1** | Ordinary archive miss; PG tries local WAL or may finish latest recovery. |
| Missing remote-required WAL/history, including post-backup bytes in a bundled segment; checksum, gzip or length corruption | **255** | Fatal recovery error, not end-of-archive. |
...
**S1 correction:** classify required coverage by timeline/LSN intervals. Local bundle coverage ends at the selected native manifest End-LSN, not the segment boundary; the final file can have padding instead of later records. Mark every segment intersecting required post-backup coverage through the frozen archive frontier remote-required, even if its filename is also bundled. Always look up the source archive first, including immediate plans: verified archive bytes return 0; transport/TLS/auth/corruption/retirement errors remain 255 regardless of local copies. Only authenticated absence can permit local fallback. For a bundled-required miss, verify that the actual local `pg_wal` file still has its selected verified bundle length/hash under current guard ownership; missing/changed/unverifiable local content or required remote coverage means 255. Optional true future/history absence remains 1. Never serve the zero-padded bundle as archive success. PostgreSQL must still reach explicit targets; no required-gap relaxation to make it succeed. [Correction experiment](review-corrections.md) proves archive-first fallback and same-segment archive preference.
...
### Target ownership before the main command (K/S2 correction)

CNPG calls [`EnsureTargetDirectoriesDoNotExist` before `Restore`](https://github.com/cloudnative-pg/cloudnative-pg/blob/4b5e244a7d031f67e025c83c1555e7726ecbbfa1/internal/cmd/manager/instance/restore/restore.go#L72-L83); its [preflight renames/removes data/WAL](https://github.com/cloudnative-pg/cloudnative-pg/blob/4b5e244a7d031f67e025c83c1555e7726ecbbfa1/pkg/management/postgres/initdb.go#L150-L194). Locking only inside the sidecar Restore RPC is therefore insufficient, even if held until its process exits. Replace that proposal with this narrow main-owned guard:

1. **Startup/argv:** install the same static executable via the existing startup-probed bin volume; bind the socket with target-write admission CLOSED. Probe checks executable/socket only, never owner acquisition, PG or restore completion. Patch only the supported recovery main command to the argument vector `[/cnpg-backup/bin/cnpg-backup, recovery-guard, --, /controller/manager, instance, restore, <unchanged CNPG-generated flags>]`; preserve any generated Args after Command. [Pinned generator](https://github.com/cloudnative-pg/cloudnative-pg/blob/4b5e244a7d031f67e025c83c1555e7726ecbbfa1/pkg/specs/jobs.go#L208-L224). No shell wrapper or arbitrary configured command. Bootstrap init only installs binaries; sidecar startup touches no target data. Reject unexpected main argv, hostPID/shareProcessNamespace or a guard that is not PID1; no extra capabilities/CGO are needed.
2. **Acquire before any CNPG preflight:** use nonblocking exclusive `flock` on stable, never-unlinked lock files in `.cnpg-backup` outside each target data directory, on every PGDATA/WAL/tablespace PVC, ordered by PVC UID. Lock descriptors are CLOEXEC. Acquire the whole immutable target set before admitting writes. Any busy lock fails without target mutation. After acquiring locks, any existing owner marker means `TargetOwnershipUncertain`, even if recorded PID/boot ID/start time indicates death; those fields are diagnostics, never takeover authority. Write/fsync fresh owner markers and parent directories before starting CNPG. Partial setup or uncertain fsync leaves markers/fails closed. Require exclusive Cluster ownership of the entire immutable volume set and working local/block lock/fsync semantics.
3. **Real local caller/seam:** guard opens one local RecoveryControl session on the existing plugin Unix socket: Begin with Cluster/operation/Pod UID and fresh guard UUID; sidecar returns its fresh process UUID. Both bind the resulting tuple. Only after acknowledged Begin does guard start CNPG. Sidecar validates matching configured Pod/Cluster/operation and owner markers before admission. Pinned CNPG Restore RPC has no tuple field: bind that RPC to this Pod-local socket's single active session; generated WAL plan/helper parameters carry the tuple. Every admitted task captures it. No rebind/reconnect to a new sidecar incarnation; control EOF/error is uncertainty: close sidecar admission, terminate/reap the main namespace, and retain markers rather than reconnect. No other daemon or network service.
4. **Lifetime:** guard remains PID1 and retains locks through all CNPG preflight, RPC/response delay, replay and shutdown. In the private main PID namespace, orphaned/detached PostgreSQL processes reparent to PID1, not just a process group; use a single child-reaping owner. After CNPG exits, stop remaining namespace descendants (`kill(-1, SIGTERM)` inside that private namespace, then bounded grace and `kill(-1, SIGKILL)`) and reap until `wait` reports no children. On guard SIGTERM do the same and report failure, not success. Do not infer termination from `pg_ctl`/CNPG success: shutdown errors can be logged without propagation. Any inability to establish complete termination poisons ownership; a timer may escalate killing, never unlock. PID1 death also causes Linux to kill its namespace members, but is **not** clean-release evidence.
5. **Drain before release:** after main descendants are gone, guard sends Drain on the same session. Sidecar irreversibly closes target/source callback admission for that tuple, rejects delayed/retried RPCs, cancels/reaps its native children, and waits for every admitted local write/task/file publication to complete. Only then may it acknowledge Drain and release its own conclusively drained source-reader hold. Pending writes prevent acknowledgment. Session/sidecar loss, canceled/ambiguous drain or changed incarnation means retain owner markers; replacement sidecar startup stays closed. A drained session never reopens. After clean drain and child termination, guard removes/fsyncs all its own markers while locks remain held, then unlocks and exits with the CNPG result (failure remains failure). Any release uncertainty is reported, not success.
6. **Crash/retry:** marker plus lock handles both live paused owners and kernel-lock release on crash. Never clear another guard's marker from PID death, Pod API deletion or elapsed time; a marker alone without live-lock exclusion is also insufficient. Retry a poisoned target with a fresh Cluster and **all fresh target PVCs**. No old sidecar can write that new target; old tuples cannot enter a cleanly released target's new session. Normal CNPG instance startup is only after the guarded recovery Job succeeds; arbitrary independent consumers/reuse of these PVCs are unsupported. S3 holds remain governed separately above.

[Model/OS evidence and limitations](review-corrections.md) cover K1/S2; the actual PID1/CNPG/sidecar implementation and namespace/drain tests are mandatory PR D/G acceptance, not completed qualification.

## 4. Minimal topology, lifecycle and mounts

Choose two released images, one Go executable with manager, instance, recovery-job, `recovery-guard` and `wal-fetch` modes:
...
2. Actual generated Pod/Job lifecycle patches with exact mount paths, restartable sidecar startup, helper executable before guarded main, quoted `%f/%p`, same UID, no credentials in argv/logs and idempotent rollout evaluation. K1/S2: pause old main before/after Restore RPC, delay response, attempt replacement preflight and assert no target mutation; sidecar/guard crashes retain markers; detached PG descendants and pending sidecar writes prevent release; clean drain permits only a new matching owner, never stale callbacks.
3. Actual Go helper/gRPC path plus real MinIO fault: post-backup required WAL 5xx/TLS/auth/timeout must end in **255 and no promotion**; authenticated future miss and verified bundle-only duplicate miss exit 1; required missing/corrupt WAL fails. Include intact bundle/no archive copy and same-segment post-EndLSN archive preference; TLS/auth/transport faults remain 255 with a bundle present. Keep the negative-control ordinary-exit test so an oracle cannot pass by never reaching post-backup replay.
