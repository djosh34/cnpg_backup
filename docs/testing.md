...
See [build-and-harness.md](build-and-harness.md#run) for prerequisites and native-only diagnostics. Check actual capabilities (Docker socket/daemon access, rootless namespaces/cgroups, kernel restrictions, CPU/RAM/disk) before choosing local real-system profiles. A missing Docker CLI or a blocked kernel feature is a **current-host limitation**, not proof local integration is permanently impossible. Host provisioning requires explicit owner authorization: the `local-runtime-4` setup task has that authorization to install supported runtime/native/race prerequisites with approved package managers or official pinned sources. Verify actual sudo access before a system transaction. Do not reset existing daemons, alter unrelated services/data, or disable/bypass seccomp, SELinux, no-new-privileges, firewall or kernel protections to force a pass. Stop an unsupported attempt, preserve its first error, and complete safe prerequisites. Run the existing harness unchanged; do not replace real acceptance with diagnostics.

**Historical assessment before the authorized setup attempt:** on the assessed Fedora worker host, unprivileged user namespaces work, but Docker/Podman/kind and Docker sockets are absent; the pinned MinIO binary fails even `--version` because route netlink is unavailable. Native PG tools can use the harness's private extracted libraries. Thus Python/Go/native diagnostics are feasible here, but real MinIO, Docker PID1/shell-free-image and kind/CNPG acceptance still need a capable host (currently hosted Linux CI). Preserve failed-attempt evidence; a native-only run never counts as MinIO/CNPG coverage. Recheck resources: this host's free disk is shared with active work and `/tmp` is tmpfs.

**Authorized retest, 2026-09-08:** this task's execution context has `NoNewPrivs=1` and seccomp filtering; `sudo -n true` fails before root execution despite the owner's passwordless-sudo authorization. Official Docker 29.8.0/rootless tools, pinned kind/kubectl and a signed-Fedora private test compiler were installed user-locally. Rootful startup requires root; rootless startup fails at `newuidmap` with `Operation not permitted`. Actual container-run/kind attempts cannot connect to a daemon. Route-netlink still returns `EAFNOSUPPORT` as user and namespace-root; real host-root netlink remains untested because sudo is blocked. MinIO startup and `--version` fail identically. Harness/unit, all-package race, native-only recovery and tight G fixtures/native probes pass; real integration, guard and CNPG smoke fail at prerequisites. These observations describe this restricted context, not every process on the host. No security policy was changed, and no MinIO/CNPG or release qualification is claimed.
