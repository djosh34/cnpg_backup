...
- Concurrent retention and backup; interrupted deletion; retained old full supporting a recent differential; last usable full retained after prolonged failure; repository-wide deletion protection through the last required WAL read, using the protocol finalized before READY. Exercise a delete already in flight when protection is requested, multiple restores, cross-cluster participants, controller/recovery crashes, paused PITR, and unsafe premature release/expiry. Uncertain/crashed restores remain protected rather than lose their hold on a timer; keep the production mechanism small and put interleaving complexity in tests. Assert rate-limited `RetentionBlocked` Warning events and durable status; event delivery is not lock state. Explicitly test the agreed truly-read-only/source-cluster-loss contract rather than assume those clients can register a hold.
