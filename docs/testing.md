...
Retention's offline regressions call the real `retention.Run` through the controlled storage seam: complete slow inventory, late list/MPU failure, valid-empty orphan-only cleanup, lost/delayed/rejected destructive responses versus restore admission, failed protection acknowledgment, cancellation, restart, 128-request/30s batching and conclusive yield. Generated fork graphs and the durable-effect oracle independently check retained inputs and replay intervals, with a negative control removing required descendant fork WAL. Manager tests connect the rendered mount to aggregate catalog/manifest reservation and test Unknown/omitted unobserved gate diagnostics. These bounded Go tests do not replace actual MinIO/CNPG recovery or workspace qualification.
