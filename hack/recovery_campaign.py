    ...
    with lockpath.open('a') as lock:
        ...
        try:
            while pending:
                ...
                if fixture and (case['group'] in ('differential', 'operations') or group in ('differential', 'operations') or
                ...
                if fixture is None:
                    ...
                    needed = {f for c in cases if any(b['status'] == 'not_executed' for b in manifest.data['scenarios'][c['id']]['branches'].values())
                              and c['group'] not in ('differential', 'operations')
                              and (plan['recipe']['layout'] != 'grouped' or c['group'] == case['group']) for f in c['fixtures']}
                    if case['group'] in ('differential', 'operations'):
                        needed = set(case['fixtures'])  # isolated live-primary mutation branch
